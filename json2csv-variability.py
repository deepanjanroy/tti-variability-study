...
network_reverse_search_metric_name = 'firstInteractive-FMP-ReverseSearchFromNetworkFirstInteractive'
...
def main():
    ...
    with open("results.csv", 'wb') as f:
        ...
        related_data_headers = ['TTFCP', 'TTFMP', 'TTI', 'TTOnload']
        ...
        for r in results:
            if len(r['failures']) != 0:
                ...
                continue
            metrics = [0, 0, 0, 0]
            netrev_histogram = get_metric_histogram(r, network_reverse_search_metric_name)
            ttonload_histogram = get_metric_histogram(r, time_to_onload_metric_name)
            diagnostics = get_diagnostic_from_all_bins(netrev_histogram)
            ...
            if len(diagnostics) == 1:
                ...
                fmp = float(d['searchBegin']['value'])
                firstInteractive = get_value(netrev_histogram)

                ttfcp = fcp - nav_start
                ttfmp = fmp - nav_start
                tti = firstInteractive - nav_start
                ttonload = get_value(ttonload_histogram)
                metrics = [ttfcp, ttfmp, tti, ttonload]
